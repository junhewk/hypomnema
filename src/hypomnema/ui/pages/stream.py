...
def _render_document_card(doc: dict[str, object]) -> None:
    ...
    with ui.card().classes("w-full mb-3 animate-fade-up cursor-pointer").style(
        ...
        ui.label(_time_ago(created_at)).classes("text-muted text-xs mt-2")
...
async def _submit_text(text: str) -> None:
    ...
    db = app.state.db
    cursor = await db.execute(
        "INSERT INTO documents (source_type, text) VALUES ('scribble', ?) RETURNING id",
        (text,),
    )
