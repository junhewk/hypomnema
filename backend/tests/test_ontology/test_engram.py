...
class TestAliasKeys:
    ...
    def test_index_alias_keys_include_english_gloss_and_legal_shortform(self) -> None:
        keys = compute_index_alias_keys("생명윤리및안전에관한법률 (bioethics and safety law)")
...
class TestGetOrCreateEngram:
    ...
    async def test_dedup_by_cosine_similarity(self, tmp_db, mock_embeddings) -> None:
        ...
        e2, c2 = await get_or_create_engram(tmp_db, "ml", "ML field", vec1)
        assert c1 is True
        assert c2 is False
        assert e1.id == e2.id
    ...
    async def test_dedup_by_concept_hash(self, tmp_db, mock_embeddings) -> None:
        ...
        await tmp_db.execute(
            "INSERT INTO engrams (canonical_name, concept_hash, description) VALUES (?, ?, ?)",
            ("ontology", concept_hash, "Study of being"),
        )
        await tmp_db.commit()
        # No embedding in engram_embeddings, so KNN won't find it, but concept hash will
        e, created = await get_or_create_engram(tmp_db, "ontology-alt", "Study of being", vec)
    ...
    async def test_different_concepts_create_separate(self, tmp_db, mock_embeddings) -> None:
        ...
        await tmp_db.commit()
        e2, c2 = await get_or_create_engram(tmp_db, "philosophy", "Study of wisdom", vecs[1])
        assert c1 is True
        assert c2 is True
        assert e1.id != e2.id
    ...
    async def test_custom_similarity_threshold(self, tmp_db, mock_embeddings) -> None:
        ...
        e2, c2 = await get_or_create_engram(tmp_db, "threshold-alt", None, vec, similarity_threshold=1.0)
    ...
    async def test_dedup_by_alias_key_for_honorific_variant(self, tmp_db, mock_embeddings) -> None:
        ...
        e1, c1 = await get_or_create_engram(tmp_db, "최지연 (prof. choi ji-yeon)", None, vecs[0])
        await tmp_db.commit()
    ...
    async def test_alias_matching_can_be_disabled_for_baseline_eval(self, tmp_db, mock_embeddings) -> None:
        ...
        e1, _ = await get_or_create_engram(tmp_db, "최지연 (prof. choi ji-yeon)", None, vecs[0])
        await tmp_db.commit()
...
class TestLinkDocumentEngram:
    ...
    async def test_creates_junction(self, tmp_db, mock_embeddings) -> None:
        ...
        engram, _ = await get_or_create_engram(tmp_db, "junction-test", None, vec)
        await tmp_db.commit()
    ...
    async def test_idempotent(self, tmp_db, mock_embeddings) -> None:
        ...
        engram, _ = await get_or_create_engram(tmp_db, "idempotent-test", None, vec)
        await tmp_db.commit()
        await link_document_engram(tmp_db, doc_row["id"], engram.id)
